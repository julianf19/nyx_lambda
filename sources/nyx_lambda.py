...
def createNotebook(nbname, functionname, nbtype, nbparameter):
    logger.info(
        "Create Notebook=%s Function=%s Type=%s Parameter=%s"
        % (nbname, functionname, nbtype, nbparameter)
    )
    ...
    if not update:
        ...
        newnb["cells"] = [{"cell_type": "markdown", "metadata": {}, "source": header}]
    ...
    if nbtype == "message":
        ...
        newnb["cells"].append(
            {"cell_type": "markdown", "metadata": {}, "source": headerfunction}
        )
...
def handleCommand(message, global_headers):
    ...
    if str(mobj["runner"]) != str(os.environ["RUNNER"]):
        ...
        if mobj["action"] == "add":
            createNotebook(
                mobj["notebook"], mobj["function"], mobj["type"], mobj["parameters"][0]
            )
        ...
        if destination in lambdasht:
            for lamb in lambdasht[destination]:
                ...
                #                    lamb["return"]=str(ret)
                    ...
                    logger_error(
                        "Lambda " + lamb["function"] + " crashed.", exc_info=True
                    )
                    tb = traceback.format_exc()
                    for trace in [
                        "TRACE:" + lamb["function"] + ":" + _ for _ in tb.split("\n")
                    ]:
                        logger_info(trace)
                        ...
                        if "<string>" in trace in trace:
                            try:
                                ...
                                logger_error(
                                    "ERROR while decoding error", exc_info=True
                                )
    ...
    try:
        ...
        if message != None:
            ...
            if "saveinput" in lamb and lamb["saveinput"]:
                file = open(
                    path + "/inputs/" + lamb["orgfunction"] + "/" + guid + ".txt", "w"
                )
                ...
                resdelete = os.system(
                    "find " + path + "/inputs/" + " -mtime +3 -delete"
                )
        ...
        es.index(index="nyx_lambdalog", id=guid, document=body)
...
def computeNewCode(notebook, newcode, common):
    allowedchards = (
        "".join([chr(_) for _ in range(65, 65 + 26)])
        + "".join([chr(_) for _ in range(48, 58)])
    ).lower()
    ...
    if startdef >= 0:
        ...
        if enddef >= 0:
            newfunc = (notebook + "_" + newcode[startdef + 5 : enddef + 3]).split("(")[
                0
            ]
            newcode2 = (
                newcode[0 : startdef + 5]
                + notebook
                + "_"
                + newcode[startdef + 5 : enddef + 3]
                + common2
                + newcode[enddef + 3 :]
            )
...
def loadConfig():
                ...
                if cell["cell_type"] == "code":
                    ...
                    if len(cell["source"]) > 1:
                        if "#@LISTEN=" in cell["source"][0]:
                            topics = (
                                cell["source"][0][9:].split("(")[0].strip().split(",")
                            )
                            ...
                            logger.info(
                                "Decoding CRON:<" + cell["source"][0][7:].strip() + ">"
                            )
                            try:
    ...
    for lamb in lambdas:
        ...
        if lamb["file"] in lambdaschanged:
            logger.info(
                "Function changed %s, do not restore next run." % (lamb["function"])
            )
        else:  # restore time
            redtime = redisserver.get(
                "lambda_"
                + str(os.environ["RUNNER"])
                + "_"
                + lamb["function"]
...
def checkIfRequirementsChanged():
    ...
    if os.path.isfile(path + "/firstrun.txt"):
...
def updateStats():
    try:  # SEND lambda updates if any
        ...
        for lamb in lambdas:
            if (
                lamb["runs"] >= 1
                or lamb.get("running_icon", "") == "regular/play-circle"
            ):
                ...
                if (
                    "starttime" in lamb
                    and lamb.get("running_icon", "") == "regular/play-circle"
                ):
                    duration = (
                        datetime.now(timezone.utc) - lamb["starttime"]
                    ).total_seconds() * 1000
                else:
                ...
                upsert = {
                    "script": {
                        ...
                        "params": {
                            ...
                            "running_icon": lamb.get(
                                "running_icon", "regular/check-circle"
                            ),
                        },
                    }
                }
                ...
                if "nextrun" in lamb:
                    ...
                    upsert["script"][
                        "source"
                    ] += "ctx._source.nextrun = params.nextrun;"
                ...
                action = {
                    "update": {
                        "_id": (
                            "R" + str(os.environ["RUNNER"]) + "_" + lamb["function"]
                        ).lower(),
                        "_index": "nyx_lambda",
                        "retry_on_conflict": 1,
                    }
...
def check_intervals_and_cron():
    ...
    while thread_check_intervals:
        ...
        for lamb in lambdas:
            ...
            if ("interval" in lamb and lamb["interval"] > 0) or (
                "crontab" in lamb and lamb["crontab"] != None
            ):
                starttime = datetime.now(timezone.utc)
                lamb["starttime"] = starttime
                if "nextrun" not in lamb:
                    ...
                    if "crontab" in lamb and lamb["crontab"] != None:
                        ...
                        lamb["nextrun"] = datetime.now(containertimezone) + timedelta(
                            seconds=lamb["crontab"].next(default_utc=False)
                        )
                    else:
                ...
                if lamb["nextrun"] <= starttime:
                    ...
                    with intervalfn_lock:
                        ...
                        logger_info(">>> Calling %s" % (lamb["function"]))
                        if "crontab" in lamb and lamb["crontab"] != None:
                            lamb["nextrun"] = datetime.now(timezone.utc) + timedelta(
                                seconds=lamb["crontab"].next(default_utc=True)
                            )
                        else:
                        ...
                        try:
                            ...
                            logger_error(
                                "Lambda " + lamb["function"] + " crashed.",
                                exc_info=True,
                            )
                            tb = traceback.format_exc()
                            for trace in [
                                "TRACE:" + lamb["function"] + ":" + _
                                for _ in tb.split("\n")
                            ]:
                                logger_info(trace)
                                if "<string>" in trace:
...
if __name__ == "__main__":
    ...
    handler = TimedRotatingFileHandler(
        "logs/" + MODULE + ".log", when="d", interval=1, backupCount=30
    )
    ...
    # >> ELK
    if os.environ.get("ELK_SSL") == "true":
        host_params = {
            "host": os.environ["ELK_URL"],
            "port": int(os.environ["ELK_PORT"]),
            "use_ssl": True,
        }
        es = ES(
            [host_params],
            http_auth=(os.environ["ELK_LOGIN"], os.environ["ELK_PASSWORD"]),
            verify_certs=False,
        )
    else:
    ...
    conn = amqstompclient.AMQClient(
        server,
        {
            "name": MODULE,
            "version": VERSION,
            "lifesign": "/topic/NYX_MODULE_INFO",
            "heartbeats": (120000, 120000),
            "earlyack": True,
        },
        QUEUE,
    ...
    logger.info("While exit, script finish...")
